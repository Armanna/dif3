...
fills AS (
    ...
    CASE
      WHEN c.claim_date_of_service::date < '2024-04-01'::date THEN -- before CostVantage
        CASE
          ...
          ELSE 'brand'
        END
      ELSE -- CostVantage effective date is 1 April 2024 but new data was laoded around 11:54 AM
        CASE
          WHEN enh.reason = 'cvs_exempt_generics' 
            THEN 'exempt generic'
  ...
  WHERE
    ...
    -- when on main bin need to split famulus from non-famulus.
...
SELECT
  ...
  "Total Full Cost" * ("Actual Effective Rate" - "Target Effective Rate"::decimal(7,5)) / 100::decimal(3, 0) as "Dollar Variance"
FROM
  aggregate_fills f
