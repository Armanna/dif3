...
-- Created      2021-04-15
-- Description  It prepares a report that includes the claims for 
--              current bi-weekly period.
-------------------------------------------------------------------------------
-- need to pick up fills >= and < date_range
with date_range as (
select 
...
reversals as (
SELECT distinct
  --changed the fields to match the contract spec in that order
  c.bin_number || '+' || c.process_control_number || '+' || c.group_id AS "BIN+PCN+GROUP",
  c.bin_number AS "Unique Identifier",
  c.npi AS "NPI",
  c.authorization_number AS "Claim Authorization Number",
