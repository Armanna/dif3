...
def draw_item(data,can,x,y,item_name,drug_type,basis_of_reimbursement_source,dollar=''):
    item = data.loc[(data['drug_type'] == drug_type) & \
         (data['basis_of_reimbursement_source'] == basis_of_reimbursement_source)].iloc[0][item_name]
...
def create_invoice(data):
    ...
    can.drawString(496, 164, '$'+ str(balance).strip())
    ...
    pdf_template = PdfFileReader(open("pdf/WalmartInvoiceTemplate.pdf", "rb"))
    ...
    output_file = "Hippo_Walmart_Invoice_" + str(invoice_number) + "_" + str(invoice_date).replace('/','') + ".pdf"
...
def run(invoice_bucket, slack_channel, slack_bot_token, hsdk_env, **kwargs):
    ...
    invoice_results = redshift_src.pull("sources/Walmart-invoice.sql")

    transaction_results, transaction_file_name = iu.extract_transactions(redshift_src, 'Walmart')  
    ...
    iu.send_file(invoice_results, "Walmart", send_file_name, file_bytes, slack_channel, slack_bot_token, invoice_bucket, hsdk_env=hsdk_env, **kwargs)    

    iu.create_and_send_file("transactions", transaction_results, transaction_file_name, slack_channel, 
                           slack_bot_token, invoice_bucket, chain = "Walmart", hsdk_env=hsdk_env, **kwargs)
