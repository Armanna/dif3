...
def transform_cvs_invoice_data(raw_claims_df, raw_ndchist_df, period_start, period_end):
    cvs = pharmacies.CVS()
    raw_invoice_claims_df = sources_claims.join_ndcs_data(raw_claims_df, raw_ndchist_df)
    # filter dataframe by chan_code, bin_number and authorization_number
    invoice_claims_df = claims.filter_by_binnumber_authnumber(raw_invoice_claims_df, cvs.BIN_NUMBERS)
    ...
    invoice_claims_df['drug_type'] = cvs.add_brand_generic_indicator_invoice(invoice_claims_df)
    ...
    raw_cvs_invoices_df = invoices.set_net_invoice_number_and_date(invoice_grouped_df, period_start, period_end, cvs.NET_NUMBER, cvs.DUE_DATE, cvs.INVOICE_STATIC_NUMBER)
    cvs_invoices_grouped_df = claims.calculate_sum_count(raw_cvs_invoices_df)
    for column in ['ingredient_cost_paid_resp','total_paid_response']:
        cvs_invoices_grouped_df[column] = utils.cast_cents_to_dollars(cvs_invoices_grouped_df, column, add_fill_reversal_sign=False)
    cvs_invoices_grouped_df = claims.calculate_claims_count(raw_cvs_invoices_df, cvs_invoices_grouped_df)
    cvs_invoices_grouped_df['grand total'] = claims.calculate_grand_total(cvs_invoices_grouped_df)  
    cvs_invoices_grouped_df = _select_columns_andapply_formatting(cvs_invoices_grouped_df)
